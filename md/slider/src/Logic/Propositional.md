...
In propositional logic, we assign **propositional variables** to represent the value of these statements. So we might make the assignments:
...
In intuitionistic logic, `¬p` is just shorthand for
...
The goal is this chapter is to define a mathematical framework in which we
prove statements by constructing proofs. In particular,
