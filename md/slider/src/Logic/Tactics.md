...
<div class="lean-code" data-start-line="50" data-end-line="50"><pre><code>#help tactic -- lists all tactics</code></pre></div>
...
<div class="lean-code" data-start-line="59" data-end-line="59"><pre><code>#help tactic apply -- tells you about a specific tactic</code></pre></div>
...
<div class="lean-code" data-start-line="68" data-end-line="71"><pre><code>variable (p : Type → Prop)
...
<div class="lean-code" data-start-line="78" data-end-line="80"><pre><code>#help tactic sorry

#print axioms my_thm1   -- &#x27;LeanW26.my_thm&#x27; depends on axioms: [sorryAx]</code></pre></div>
...
<div class="lean-code" data-start-line="88" data-end-line="92"><pre><code>theorem t (x y z : ℚ) (h1 : 2*x &lt; 3*y) (h2 : -4*x + 2*z &lt; 0) (h3 : 12*y - 4* z &lt; 0)
  : False := by
  linarith
...
<div class="lean-code" data-start-line="106" data-end-line="108"><pre><code>example : (¬ ∃ x, p x) → (∀ x, ¬ p x) := by
  intro hnep x
  sorry                -- ⊢ ¬p x</code></pre></div>
...
<div class="lean-code" data-start-line="118" data-end-line="128"><pre><code>example : (¬ ∃ x, p x) ↔ (∀ x, ¬ p x) := by
  apply Iff.intro
  · intro h x hp               --  ⊢ False
    apply h                    --  ⊢ ∃ x, p x
...
<div class="lean-code" data-start-line="148" data-end-line="156"><pre><code>theorem my_thm2 (q : Prop) : q → q := id
...
example (x y : ℕ) : x = y → y = x := by
  apply Eq.symm</code></pre></div>

 If you are stuck, there is `apply?` 

<div class="lean-code" data-start-line="160" data-end-line="163"><pre><code>example (x y : ℕ) : x = y → y = x := by
  apply?         -- Try this:
                 --   [apply] exact fun a ↦ id
                 -- (Eq.symm a)</code></pre></div>
...
<div class="lean-code" data-start-line="179" data-end-line="186"><pre><code>example : (¬ ∃ x, p x) ↔ (∀ x, ¬ p x) := by
  apply Iff.intro
  · intro h x hp
  ...
  · intro h hepx
    ...
    intro x hpa
    exact (h x) hpa</code></pre></div>

 And even more compactly using structure notation and matching 

<div class="lean-code" data-start-line="190" data-end-line="195"><pre><code>example : (¬ ∃ x, p x) ↔ (∀ x, ¬ p x) := by
  apply Iff.intro
  · intro h x hp
    exact h ⟨ x, hp ⟩
  · intro h ⟨ x, hp ⟩
    exact (h x) hp</code></pre></div>
...
<div class="lean-code" data-start-line="203" data-end-line="212"><pre><code>example : ∀ (x : ℕ), ∃ y, x &lt; y := by
  intro x
  use x+1
  apply?       -- Try this:
...
example : ∀ (x : ℕ), ∃ y, x &lt; y := by
  intro x
  use x+1
  exact lt_add_one x</code></pre></div>

 `use` allows for multiple introductions at the same time

<div class="lean-code" data-start-line="216" data-end-line="218"><pre><code>example : ∃ (x:ℕ), ∃ y, x &lt; y := by
  use 0, 1
  exact Nat.one_pos</code></pre></div>
...
<div class="lean-code" data-start-line="230" data-end-line="232"><pre><code>example (c : Type) (h : p c) : ∃ x, p x := by
  apply Exists.intro c
  assumption</code></pre></div>
...
<div class="lean-code" data-start-line="241" data-end-line="245"><pre><code>variable (P Q : Type → Prop)

example : (∃ x, P x ∧ Q x) →  ∃ x, Q x ∧ P x := sorry

example : (∃ x, P x ∨ Q x) →  ∃ x, Q x ∨ P x := sorry</code></pre></div>
...
<div class="lean-code" data-start-line="260" data-end-line="275"><pre><code>variable (p : Type → Prop)
variable (r : Prop)
...
<div class="lean-code" data-start-line="283" data-end-line="286"><pre><code>example (p q : Prop) : p ∧ q → p ∨ q := by
  intro ⟨ h1, h2 ⟩
  have hp : p := h1
  exact Or.inl hp</code></pre></div>

 If you need an intermediate value, you should use `let`. 

<div class="lean-code" data-start-line="290" data-end-line="292"><pre><code>example : ∃ n , n &gt; 0 := by
  let m := 1
  exact ⟨ m, Nat.one_pos ⟩</code></pre></div>
...
<div class="lean-code" data-start-line="300" data-end-line="306"><pre><code>example (p q : Prop) : (p ∨ q) → q ∨ p  := by
  intro h
  apply Or.elim h
  · intro hp
    exact Or.symm h
  · intro hq
    exact Or.symm h</code></pre></div>

 Becomes 

<div class="lean-code" data-start-line="310" data-end-line="314"><pre><code>example (p q : Prop) : (p ∨ q) → q ∨ p  := by
  intro h
  cases h with
  | inl hp =&gt; exact Or.inr hp
  | inr hq =&gt; exact Or.symm (Or.inr hq)</code></pre></div>
...
<div class="lean-code" data-start-line="325" data-end-line="325"><pre><code>variable (P Q : Type → Prop)</code></pre></div>

 Cases on an Exists structure 

<div class="lean-code" data-start-line="329" data-end-line="332"><pre><code>example : (∃ x, P x ∧ Q x) → ∃ x, Q x ∧ P x := by
  intro h
  cases h with
  | intro x h =&gt; exact ⟨ x, And.symm h ⟩</code></pre></div>

 Cases on an And structure 

<div class="lean-code" data-start-line="336" data-end-line="339"><pre><code>example (p q : Prop) : (p ∧ q) → (p ∨ q) :=  by
  intro h
  cases h with
  | intro hp hq =&gt; exact Or.inl hp</code></pre></div>
...
<div class="lean-code" data-start-line="349" data-end-line="350"><pre><code>def False.elim {p : Prop} (h : False) : p :=
  nomatch h</code></pre></div>

 We could have used the `cases` tactic, which finds no cases and
produces a term. 

<div class="lean-code" data-start-line="355" data-end-line="357"><pre><code>def False.elim&#x27; {p : Prop} (h : False) : p :=
  by cases h
  -- ... no cases</code></pre></div>

 The term level proov uses t version of the recursor for `False` 

<div class="lean-code" data-start-line="361" data-end-line="367"><pre><code>#print False.elim&#x27;   -- fun {p} h ↦ False.casesOn (fun t ↦ h = t → p) h (Eq.refl h)
...
<div class="lean-code" data-start-line="374" data-end-line="378"><pre><code>--hide
...
<div class="lean-code" data-start-line="382" data-end-line="386"><pre><code>def on_right (p q : Person) : Prop := match p with
  | mary =&gt; q = steve
  | steve =&gt; q = ed
  | ed =&gt; q = jolin
...
<div class="lean-code" data-start-line="390" data-end-line="400"><pre><code>example : ∀ x : Person, ∃ y, ¬on_right x y := by
  intro hp
  cases hp with
...
<div class="lean-code" data-start-line="411" data-end-line="416"><pre><code>theorem cases_example (p : Prop) : p ∨ ¬p := by
  by_cases h : p
  · exact Or.inl h -- show p ∨ ¬p assuming h : p
  · exact Or.inr h -- show p ∨ ¬p assuming h : ¬p

#print axioms cases_example    --  [propext, Classical.choice, Quot.sound]</code></pre></div>

 However, `by_cases` on a non-prop usually does not require classical reasoning. 

<div class="lean-code" data-start-line="420" data-end-line="427"><pre><code>theorem another_example : ∀ n : ℕ, n = 0 ∨ n &gt; 0 := by
  intro n
  by_cases h : n = 0
  · exact Or.inl h
  · apply Or.inr
...
#print axioms another_example      -- does not depend on any axioms</code></pre></div>
...
<div class="lean-code" data-start-line="438" data-end-line="438"><pre><code>def next_to (p q : Person) := on_right p q ∨ on_right q p</code></pre></div>


We might encounter `next_to` in a proof and want to replace it with its definition.


<div class="lean-code" data-start-line="444" data-end-line="447"><pre><code>example : ∀ p , ∀ q , (on_right p q) → next_to p q := by
  intro p q h
  unfold next_to             -- helps us see what we have to do
  exact Or.inl h</code></pre></div>
...
<div class="lean-code" data-start-line="458" data-end-line="459"><pre><code>example : ∀ p : Person, ∃ q : Person, next_to p q := sorry
...
<div class="lean-code" data-start-line="469" data-end-line="482"><pre><code>def E (n : Nat) : Prop := match n with
  | Nat.zero =&gt; True
  | Nat.succ x =&gt; ¬E x
...
<div class="lean-code" data-start-line="488" data-end-line="491"><pre><code>#print even_or_even_succ    -- ∀ (n : ℕ), E n ∨ E n.succ :=
                            -- fun n ↦ Nat.recAux (Or.inl True.intro)
                            -- (fun k ih ↦ Or.elim ih (fun h ↦ Or.inr
                            -- fun a ↦ a h) fun h ↦ Or.inl h) n</code></pre></div>
...
<div class="lean-code" data-start-line="502" data-end-line="539"><pre><code>example {p q : Prop} : p ∧ q → q ∧ p := by
  intro hpq
  induction hpq with
...
<div class="lean-code" data-start-line="551" data-end-line="551"><pre><code>def no_negs ( x : PreDyadic ) : Prop := sorry</code></pre></div>
...
<div class="lean-code" data-start-line="559" data-end-line="559"><pre><code>example (x : PreDyadic) : no_negs x → no_negs (double x) := sorry</code></pre></div>


c) Prove


<div class="lean-code" data-start-line="565" data-end-line="565"><pre><code>example (x y : PreDyadic) : no_negs x → no_negs y → no_negs (mul x y) := sorry</code></pre></div>
...
<div class="lean-code" data-start-line="576" data-end-line="576"><pre><code>#help tactic</code></pre></div>
...
<div class="lean-code" data-start-line="591" data-end-line="606"><pre><code>open Lean Elab Tactic
...
example : 0 = 0 := by
  my_tactic             -- yay it works!</code></pre></div>
...
<div class="lean-code" data-start-line="618" data-end-line="620"><pre><code>--hide
end LeanW26
--unhide</code></pre></div>
